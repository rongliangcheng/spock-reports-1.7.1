...
class HtmlReportCreator extends AbstractHtmlCreator<SpecData>
    ...
    protected String reportHeader( SpecData data ) {
        ...
        String title = Utils.specAnnotation(data,Title).value()
        if( title != "" )
            return "报告：${Utils.specAnnotation(data,Title).value()}";
        else
            return "Report for ${Utils.getSpecClassName( data )}"
    }

    @Override
    void writeSummary( MarkupBuilder builder, SpecData data ) {
        builder.div( 'class': 'back-link' ) {
            a( href: 'index.html', '<< 返回' )
        }
            ...
            table( 'class': 'summary-table' ) {
                ...
                tbody {
                    tr {
                        ...
                        //reportAggregator?.jsonListReport(data,stats)
                    }
                }
            }
        }
...
//        }
    ...
    private void writeFeature( MarkupBuilder builder, SpecData data ) {
        ...
        for ( FeatureInfo feature in data.info.allFeaturesInExecutionOrder ) {
            ...
            //Add some space between different blocks
            builder.tr{
                builder.td{
                    builder.br "  "
                    builder.br "  "
